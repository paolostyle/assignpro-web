...
export const storeActions = {
    ...
    sendData({commit, getters, dispatch}, id) {
        ...
        return axios.post('/solve', requestData)
    ...
    saveHistoryToDatabase({state, getters}, date) {
        if (state.user) {
            firebase.database().ref(getters.getDatabasePath).set({
                historyDate: date,
